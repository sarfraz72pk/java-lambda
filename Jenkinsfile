        ...
        stage('Deploy to QA') {
            ...
            steps {
                script {
                    ...
                    sh "aws s3 cp target/${JARNAME} s3://bermtec229/lambda-test/"
            ...
            steps {
                script {
                    if (env.BRANCH_NAME == "master") {
                        ...
                        sh "aws s3 cp target/${JARNAME} s3://bermtec229/lambda-prod/"
                        //  sh './deploy-test.sh $AWS_ACCESS_KEY $AWS_SECRET_KEY'
                        // if (does_lambda_exist('prodfunction')) {
                            sh "aws lambda update-function-code --function-name prodfunction --s3-bucket bermtec229 --s3-key lambda-prod/${JARNAME}"
                        //}  
                    }
                }
            }
